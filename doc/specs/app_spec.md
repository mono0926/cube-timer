...
| 状態 (State) | 画面表示テキスト     | 色 (Text/Pad)          | トリガー/動作                                                              | 音声        |
| ------------ | -------------------- | ---------------------- | -------------------------------------------------------------------------- | ----------- |
| **Idle**     | "タッチしてスタート" | Default (白/黒) / Grey | 初期状態。画面タッチで `Holding` へ遷移。                                  | -           |
| **Holding**  | "そのまま..."        | Red                    | 指を置いている間。0.3秒経過で `Ready` へ遷移。途中で離すと `Idle` へ戻る。 | -           |
| **Ready**    | "よーい"             | Green                  | 計測準備完了。指を離した瞬間に `Running` へ遷移。                          | `ready.mp3` |
| **Running**  | "スタート"           | Default (Glow)         | 計測中。画面タップで `Stopped` へ遷移。                                    | `start.mp3` |
| **Stopped**  | "ストップ"           | Default                | 計測終了。リセットボタンで `Idle` へ戻る。                                 | `stop.mp3`  |
