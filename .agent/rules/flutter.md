...
## タスク完了時の必須チェックリスト (Quality Assurance)

コードの変更を伴う成果物を提示する際（タスク完了報告やレビュー依頼時）に、必ず以下のプロセスを実行して品質を担保してください。
※ 質問のみの場合や、エラー解決の相談をする場合などは対象外です。

1.  **静的解析の完全解消 (`dart analyze`)**
    - プロジェクト全体で `dart analyze` を実行する。
    - **すべての** エラー・警告・Info（Lint指摘）を解消する。「これくらいは良いだろう」という判断はしない。
    - `dart fix --apply` を活用・推奨するが、それで直らないものも手動で修正する。
    - どうしても解消できない理由がある場合のみ、ユーザーに理由を説明して承認を得た上で `ignore` コメントを入れる（基本は解消する）。

2.  **フォーマットの適用 (`dart format`)**
    - `dart format .` を実行し、全ファイルのフォーマットを整える。

3.  **ビルド・実行確認**
    - コード変更がビルドを壊していないことを確認する。
